    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
        <span style={{ fontSize: 12, color: "var(--text-faint)", fontFamily: "monospace" }}>
          {toDisplayText(grievance.grievance_id, "—")}
        </span>
        <span className="badge badge-neutral">{toDisplayText(CATEGORY_LABELS[grievance.category] || grievance.category)}</span>
        {grievance.incident_id && <span className="badge badge-status">Community Incident</span>}
      </div>
      <div style={{ fontWeight: 700, fontSize: 14.5, marginBottom: 4 }}>{toDisplayText(grievance.title, "Untitled grievance")}</div>
      <div style={{ display: "flex", gap: 14, fontSize: 12.5, color: "var(--text-muted)", flexWrap: "wrap" }}>
        <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
          <MapPin size={12} /> {toDisplayText(grievance.location?.address || grievance.department, "Location not provided")}
        </span>
        <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
